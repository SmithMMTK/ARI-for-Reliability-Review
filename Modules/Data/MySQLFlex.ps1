...
If ($Task -eq 'Processing') {
    ...
    $MySQL = $Resources | Where-Object { $_.TYPE -eq 'microsoft.dbformysql/flexibleservers' }
...
Else {
    ...
    if ($SmaResources.MySQLFlex) {

        $TableName = ('MySQLTableFlex_'+($SmaResources.MySQLFlex.id | Select-Object -Unique).count)
        ...
        $ExcelVar = $SmaResources.MySQLFlex

        $ExcelVar | 
        ForEach-Object { [PSCustomObject]$_ } | Select-Object -Unique $Exc | 
        Export-Excel -Path $File -WorksheetName 'MySQLFlex' -AutoSize -MaxAutoSizeRows 100 -TableName $TableName -TableStyle $tableStyle -ConditionalText $condtxt -Style $Style
